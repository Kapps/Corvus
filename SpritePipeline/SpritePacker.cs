...
/**
* 
*	This class is almost enitrely stolen from Microsoft's SpriteSheet sample. 
...
namespace SpritePipeline {
	/// <summary>
	/// Helper used to combine multiple sprites into a single larger sprite with multiple frames.
	/// </summary>
	public static class SpritePacker {
		...
		public static BitmapContent PackSprites(IList<BitmapContent> sourceSprites, ICollection<Rectangle> outputSprites, ContentProcessorContext context) {
			...
			context.Logger.LogImportantMessage("Packed {0} sprites into a {1}x{2} sheet, {3}% efficiency", sprites.Count, outputWidth, outputHeight, totalSpriteSize * 100 / outputWidth / outputHeight);
		...
		/// </summary>
		static BitmapContent CopySpritesToOutput(List<ArrangedSprite> sprites, IList<BitmapContent> sourceSprites, ICollection<Rectangle> outputSprites, int width, int height) {
